...
const selectedTerm = computed(
  () => terms.value.find((t) => t.id === selectedId.value) ?? null,
);
...
const requiredLevel = computed(() =>
  Math.max(selectedTerm.value?.minRoleLevel ?? 0, form.minRoleLevel),
);
const canManage = computed(
  () => (auth.user?.roleLevel ?? 0) >= requiredLevel.value,
);
...
function syncListsFromText() {
  form.aliases = aliasesText.value
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  form.relatedTables = relatedTablesText.value
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
}
...
async function save() {
  ...
  try {
    if (isNew.value) {
      ...
      terms.value = [...terms.value, created].sort((a, b) =>
    ...
    errorMessage.value =
      e instanceof ApiError ? e.message : "저장 중 오류가 발생했습니다.";
  } finally {
    saving.value = false;
  }
}
...
async function remove() {
  ...
  try {
    ...
    errorMessage.value =
      e instanceof ApiError ? e.message : "삭제 중 오류가 발생했습니다.";
  } finally {
    saving.value = false;
  }
}
...
<template>
  <div class="pane">
    <div class="pane-inner">
      <h1>업무 용어</h1>
      <p class="sub">
        회사에서 쓰는 말을 등록해 두면 질문할 때 그 기준으로 계산합니다. 여기
        적힌 내용이 답변 정확도를 그대로 결정합니다.
      </p>
      ...
      <div class="split">
        ...
        <div class="col-form">
            ...
            <div class="panel-b">
              ...
              <div class="frow">
                ...
                <input
                  v-model="aliasesText"
                  class="field fv"
                  placeholder="쉼표로 구분, 예: 휴면 고객, 떠난 고객"
                />
              </div>
              <div class="frow">
                <label>정의</label>
                <textarea
                  ...
                  class="field fv tall"
                  rows="3"
                  placeholder="이 용어를 어떻게 계산할지 설명"
                />
              </div>
              <div class="frow">
                <label>계산 기준</label>
                <input
                  v-model="form.sqlHint"
                  class="field fv mono"
                  placeholder="예: MAX(o.ordered_at) < CURRENT_DATE - INTERVAL '90 days'"
                />
              </div>
              <div class="frow">
                <label>관련 테이블</label>
                <input
                  v-model="relatedTablesText"
                  class="field fv"
                  placeholder="쉼표로 구분, 예: ord, region"
                />
              </div>
              <div class="frow">
                <label>공개 범위</label>
              ...
              <p v-if="!canManage" class="error-text">
                이 등급(레벨 {{ requiredLevel }}) 용어를 관리하려면 내 등급이 그
                이상이어야 합니다.
              </p>
          ...
          <p class="note">
            등록 항목이 많아지면 전부 불러오지 않고 질문과 관련된 용어만 찾아
            쓰는 방식으로 바뀔 수 있습니다. 이 화면은 그대로입니다.
          </p>
        </div>
      </div>
    </div>
...
.list-panel {
  ...
  width: 100%;
  /* col-form 쪽 자연 높이에 맞춰 stretch되고, 그 안에서 목록만 스크롤된다 */
  max-height: 530px;
}
.term-scroll {
  overflow-y: auto;
